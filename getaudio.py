...
class GetAudio:
    ...
    def __init__(self, csv_filepath, destination_folder= 'audio/', wait= 1.5, debug=True ):
        '''
    ...
    def get_audio(self):
        ...
        for lang_num in self.audio_df['language_num']:
            if not os.path.exists(self.destination_folder +'{}.wav'.format(lang_num)):
                ...
                # print(urllib.request.urlretrieve(self.url.format(lang_num)))
                # (filename, headers) = urllib.request.urlretrieve(self.url.format(lang_num))
                with urllib.request.urlopen(self.url.format(lang_num)) as response, open('Request Data\\{}'.format(lang_num), 'wb') as out_file:
                    shutil.copyfileobj(response, out_file)
                sound = AudioSegment.from_mp3('Request Data\\{}'.format(lang_num)) #mp3 to wav
